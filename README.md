...
- Retention enforced at the configured deadline, defaulting to midnight **after** the event in `America/Los_Angeles`. PostgreSQL cron deletes expired events every minute, cascading to their RSVPs and mail jobs. Public and admin reads deny access at the deadline even before deletion runs.
